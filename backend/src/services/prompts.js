...
function buildEnvBlock(environment) {
  ...
  return `\nSite environment: ${env} — this is NOT a finished live website. Detected signals:\n${sigs}\n` +
    ...
    `- Keep the score focused on REAL defects that are wrong regardless of launch status: broken functionality, JS/console errors, broken links, accessibility problems, security issues.\n`
}
...
export function buildSystemPrompt(module, checks, requiredTools = [], extraInstructions = '', environment = null) {
  ...
  const base = `You are a Principal QA Engineer with 15+ years auditing production websites. You are meticulous, evidence-driven, and never guess. You have access to real Playwright browser tools running in a real Chromium browser. Use them to thoroughly test the given URL.

Checks requested: ${checks.join(', ')}
${requiredBlock}${envBlock}${extraBlock}
