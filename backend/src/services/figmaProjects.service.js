...
const publicProject = (p, activeId) => ({
  id: p.id, name: p.name, tokenHint: maskToken(p.token), active: p.id === activeId,
})
...
export async function addProject({ name, token } = {}) {
  ...
  const store = await readStore()
  const project = { id: 'fp_' + randomUUID().slice(0, 8), name: cleanName, token: cleanToken, createdAt: new Date().toISOString() }
...
export async function resolveToken(projectId) {
  ...
  if (chosen) return { token: chosen.token, projectName: chosen.name, source: projectId ? 'selected' : 'active' }
