...
async function readStore() {
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      if (parsed.items || parsed.disabled)
        return { items: parsed.items || {}, disabled: parsed.disabled || {} }
...
function normalizeItem(raw = {}) {
  ...
  const item = {
    ...
    tools: Array.isArray(raw.tools)
      ? raw.tools.filter((t) => typeof t === 'string').slice(0, 10)
      : [],
  }
  if (type === 'dropdown') {
    item.options = (Array.isArray(raw.options) ? raw.options : [])
      .map((o) => String(o).trim().slice(0, 60))
      .filter(Boolean)
      .slice(0, 20)
    if (item.options.length < 2) throw new Error('a dropdown needs at least 2 options')
    item.default =
      typeof raw.default === 'string' && item.options.includes(raw.default) ? raw.default : ''
  } else {
...
export async function setBuiltinDisabled(moduleId, checkId, disabled) {
  ...
  if (disabled) set.add(String(checkId))
  else set.delete(String(checkId))
