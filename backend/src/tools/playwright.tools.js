  ...
  return withPage(url, async (page) => {
    const forms = await page.evaluate(() =>
      [...document.querySelectorAll('form')].map((form, idx) => ({
        ...
        hasRecaptcha:
          form.innerHTML.includes('recaptcha') || form.innerHTML.includes('g-recaptcha'),
      })),
    )
...
export async function getWebVitals(url) {
  return withPage(url, async (page) => {
    ...
    const grade = (n) =>
      n === undefined ? 'N/A' : n < 1800 ? 'good' : n < 3000 ? 'needs-improvement' : 'poor'
    ...
    return {
      ...
      metrics: {
        ...
        ttfb: {
          value: vitals.ttfb,
          unit: 'ms',
          status: vitals.ttfb < 800 ? 'good' : 'needs-improvement',
        },
...
export async function detectTracking(url) {
  return withPage(url, async (page) => {
    const tracking = await page.evaluate(() => {
      ...
      return {
        ga4:
          scripts.some((s) => /googletagmanager|google-analytics/.test(s)) ||
          html.includes('gtag('),
        ...
        intercom:
          scripts.some((s) => /intercom/.test(s)) || html.includes('window.intercomSettings'),
    ...
    const a11y = await page.evaluate(() => {
      ...
      if (noAlt.length)
        issues.push({
          ...
          count: noAlt.length,
          severity: 'error',
          message: `${noAlt.length} images missing alt attribute`,
        })
      ...
      if (noText.length)
        issues.push({
          ...
          count: noText.length,
          severity: 'error',
          message: `${noText.length} buttons with no text or aria-label`,
        })

      // H1 count
      const h1s = document.querySelectorAll('h1')
      if (h1s.length === 0)
        issues.push({ type: 'no-h1', severity: 'error', message: 'No H1 heading found' })
      if (h1s.length > 1)
        issues.push({
          type: 'multi-h1',
          severity: 'warn',
      ...
      if (unlabelled.length)
        issues.push({
          ...
          severity: 'error',
          message: `${unlabelled.length} inputs without labels`,
        })
    ...
    return {
      ...
      score: Math.round(
        Math.max(
          0,
          100 -
            a11y.issues.filter((i) => i.severity === 'error').length * 15 -
            a11y.issues.filter((i) => i.severity === 'warn').length * 5,
        ),
      ),
  ...
  return withPage(url, async (page) => {
    ...
    const meta = await page.evaluate(() => {
      ...
      const visible = (el) => {
        ...
        return (
          r.height > 60 && r.width > vw * 0.4 && s.display !== 'none' && s.visibility !== 'hidden'
        )
      }
      ...
      const checksFor = (el, tag) => {
        ...
        if (btns.length)
          checks.push({
            ...
            detail:
              btnNoLabel === 0 ? 'all buttons labelled' : `${btnNoLabel} button(s) with no label`,
          })

        const links = [...el.querySelectorAll('a[href]')]
        const linkNoText = links.filter(
          (a) =>
            !a.textContent.trim() &&
        ...
        if (inputs.length) {
          ...
          const noLabel = inputs.filter(
            (f) =>
              !f.getAttribute('aria-label') &&
              !f.getAttribute('aria-labelledby') &&
              !f.closest('label') &&
              !(f.id && labelFors.includes(f.id)),
          ).length
          checks.push({
      ...
      for (const s of picked) {
        s.verdict = s.checks.some((c) => c.status === 'fail')
          ? 'fail'
      ...
      if (withShots) {
        try {
          const buf = await page
            .locator(`[data-qa-section="${s.index}"]`)
            .first()
            .screenshot({ timeout: 8000 })
          base64 = buf.toString('base64')
        } catch {
          /* element not screenshottable — keep metadata only */
        }
      }
    ...
    if (withShots) {
      try {
        fullPage = (await page.screenshot({ fullPage: true })).toString('base64')
      } catch {
        /* ignore */
      }
    }
...
export async function captureFindingHighlights(url, targets = []) {
  ...
  const valid = (targets || [])
    .filter(
      (t) =>
        t &&
        ((t.selector && typeof t.selector === 'string') ||
          (t.textMatch && typeof t.textMatch === 'string')),
    )
    .slice(0, 8)
  ...
  return withPage(url, async (page) => {
    ...
    const resolveLocator = async (t) => {
      if (t.selector) {
        try {
          ...
          /* invalid selector syntax — fall through to text */
        }
      }
      if (t.textMatch) {
    ...
    for (const t of valid) {
      try {
        ...
        /* selector unusable / element not screenshottable — skip */
      }
    }
    return out
  })
...
export async function detectEnvironment(url) {
  ...
  try {
    const resp = await page.goto(url, {
      ...
      timeout: config.playwright.navTimeoutMs,
    })
    if (resp) {
    ...
    dom = await page.evaluate(() => {
      ...
      return {
        ...
        comingSoon:
          /coming soon|under construction|launching soon|site is under|maintenance mode|be right back|opening soon|website is being/.test(
            bodyText,
          ),
        lorem: /lorem ipsum|dolor sit amet/.test(bodyText),
        placeholderImgs: imgs.filter((i) =>
          /placeholder|placehold\.|via\.placeholder|dummyimage|lorempixel|picsum\.photos/.test(
            i.src || '',
          ),
        ).length,
        phpErrors:
          /(notice|warning|deprecated|fatal error)\s*:/.test(bodyText) ||
          /undefined (index|variable|array key|offset)/.test(bodyText),
        hasAnalytics: /gtag\(|googletagmanager|google-analytics|_gaq|fbq\(/.test(
  ...
  const HOST_PLATFORM =
    /netlify\.app|vercel\.app|herokuapp\.com|pages\.dev|web\.app|firebaseapp\.com|ngrok\.|\.sites\.|pantheonsite\.io|wpengine\.com|kinsta\.cloud|cloudwaysapps\.com|myftpupload\.com|onrocket/

  const signals = []
  const add = (signal, weight, detail) => signals.push({ signal, weight, detail })

  if (HOST_DEV.test(host))
    add('local-host', 'development', `Host "${host}" is a local/development address`)
  if (HOST_STAGING.test(host))
    add('staging-host', 'staging', `Host "${host}" looks like a staging/test environment`)
  if (HOST_PLATFORM.test(host))
    add(
      'preview-host',
      'staging',
      `Host "${host}" is a temporary/preview hosting domain, not a custom production domain`,
    )
  if (noindex)
    add(
      'noindex',
      'staging',
      'Page is set to noindex (robots meta / X-Robots-Tag) — search engines are blocked, typical of a non-public site',
    )
  if (dom.comingSoon)
    add(
      'coming-soon',
      'maintenance',
      'Page shows "coming soon" / "under construction" / "maintenance" text',
    )
  if (dom.lorem)
    add('placeholder-text', 'development', 'Lorem ipsum placeholder text found in the content')
  if (dom.placeholderImgs > 0)
    add(
      'placeholder-images',
      'development',
      `${dom.placeholderImgs} placeholder/dummy image(s) found`,
    )
  if (dom.phpErrors)
    add(
      'server-errors',
      'development',
      'Server notices/warnings are visible on the page (debug mode left on)',
    )
  if (typeof dom.wordCount === 'number' && dom.wordCount < 60 && !dom.comingSoon)
    add(
      'thin-content',
      'development',
      `Very little text on the page (${dom.wordCount} words) — may be unfinished`,
    )
