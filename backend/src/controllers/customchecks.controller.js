...
export async function listCustomChecks(_req, res) {
  try {
    res.json(shape(await getCustomChecks()))
  } catch (err) {
    console.error('[custom-checks] list error:', err)
...
export async function postCustomCheck(req, res) {
  try {
    ...
    console.error('[custom-checks] add error:', err)
...
export async function patchCustomCheck(req, res) {
  try {
    res.json(shape(await updateCustomCheck(req.params.moduleId, req.params.id, req.body || {})))
  } catch (err) {
    console.error('[custom-checks] update error:', err)
    res.status(400).json({ error: err.message })
  }
}
...
export async function deleteCustomCheck(req, res) {
  try {
    res.json(shape(await removeCustomCheck(req.params.moduleId, req.params.id)))
  } catch (err) {
    console.error('[custom-checks] delete error:', err)
    res.status(400).json({ error: err.message })
  }
}
...
export async function setBuiltinDisabledHandler(req, res) {
  try {
    ...
    console.error('[custom-checks] builtin error:', err)
    res.status(400).json({ error: err.message })
  }
}
