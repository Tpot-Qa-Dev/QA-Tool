...
app.listen(config.port, () => {
  ...
  console.log(`  Mode: ${config.env.toUpperCase()} ${config.isDev ? '(verbose logs · permissive CORS · detailed errors)' : '(minimal logs · locked CORS · generic errors)'}`)
  console.log(`  API keys: ${Object.entries(status).map(([k, v]) => `${k}: ${v}`).join('  ·  ')}`)
