...
export function mergeReports(reports) {
  ...
  for (const r of valid) {
    ...
    if (r.usage) {
      ...
      merged.usage.totalTokens  += r.usage.totalTokens  || (r.usage.inputTokens || 0) + (r.usage.outputTokens || 0)
      merged.usage.calls        += r.usage.calls        || 0
    }
  }

  merged.overallScore = scores.length ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0
