...
export function reportSections(report) {
  ...
  if (report.screenshots?.length)
    secs.push({ key: 'screenshots', label: 'Screenshots', kind: 'special' })
  ...
  if (report.criticalIssues?.length)
    secs.push({ key: 'criticalIssues', label: 'Critical Issues', kind: 'list' })
  if (report.positives?.length)
    secs.push({ key: 'positives', label: 'What’s Working', kind: 'list' })
...
function listBlock(report, key) {
  ...
  if (key === 'nextSteps') {
    const rows = report.nextSteps
      .map(
        (s) =>
          `<li>→ ${esc(s.step)} ${s.timeline ? `<span class="owner">${esc(s.timeline)}</span>` : ''}</li>`,
      )
      .join('')
...
function envBannerHtml(environment) {
  ...
  const sig = environment.signals?.length
    ? `<span class="sig">Signals: ${esc(environment.signals.map((s) => s.signal).join(' · '))}</span>`
    : ''
  const note = live
    ? ''
    : 'Reviewed as <b style="display:inline;color:inherit">pre-launch</b> — not marked down for noindex, missing analytics, or placeholder content.'
...
function buildReportHtml(report, reportId) {
  let blocks = reportSections(report)
...
export function exportHtmlReport(report, reportId, fileName) {
  downloadBlob(
    new Blob([buildReportHtml(report, reportId)], { type: 'text/html' }),
...
export function exportPdfReport(report, reportId, fileName) {
  ...
  const html = buildReportHtml(report, reportId).replace(
    /<title>[\s\S]*?<\/title>/,
...
export function exportHtmlReportSelective(report, reportId, selectedKeys) {
  ...
  downloadBlob(
    new Blob([renderDoc(report, reportId, body)], { type: 'text/html' }),
    `qa-report-${reportId}-custom.html`,
  )
}
...
const sectionAspects = (s, show) => {
  ...
  rows.push([
    'Elements',
    `${c.links} links · ${c.buttons} btn · ${c.images} img · ${c.headings} headings${c.forms ? ` · ${c.forms} form` : ''}`,
  ])
  return rows
}
...
function sectionCardHtml(s, n, checks) {
  ...
  const checkList = (s.checks || [])
    .map(
      (c) => `<li class="chk ${VBADGE[c.status]}">
      <span class="chk-i">${VICON[c.status] || '•'}</span><b>${esc(c.label)}</b> — ${esc(c.detail)}</li>`,
    )
    .join('')
  const verdict = s.verdict
    ? `<span class="badge ${VBADGE[s.verdict]}">${VICON[s.verdict]} ${esc(s.verdict)}</span>`
    : ''
  ...
  const rows = hasMeasured
    ? sectionAspects(s, aspectsForChecks(checks))
        .map(([k, v]) =>
          hasFigma
...
export function exportSectionHtml(report, reportId = report.url) {
  ...
  const summaryRows = (report.sections || [])
    .map((s, i) => {
...
<footer>Generated by QA Automation Tool v1.0 · section-by-section (web)</footer>
  ...
  downloadBlob(
    ...
    `section-report-${(reportId || 'web').replace(/[^a-z0-9]+/gi, '-').slice(0, 40)}.html`,
  )
}
...
export function exportCsvReport(report, reportId, fileName) {
  ...
  downloadBlob(
    ...
    buildName(fileName, `qa-report-${reportId}`, 'csv'),
  )
}
...
export function exportJsonReport(report, reportId, fileName) {
  ...
  downloadBlob(
    ...
    buildName(fileName, `qa-report-${reportId}`, 'json'),
  )
}
