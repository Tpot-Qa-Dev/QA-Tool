    ...
    <AppBar position="sticky" sx={{ borderRadius: 0, mb: 3, top: 0, zIndex: 5 }}>
      <Toolbar disableGutters sx={{ gap: 1, flexWrap: 'wrap', px: { xs: 2, md: 3 }, py: 1 }}>
        ...
        >
          ...
          {health?.model && (
            <Tooltip title="Audit model">
              <Chip size="small" variant="outlined" label={health.model.replace(/^claude-/, '')} />
            </Tooltip>
          )}
          {health && typeof health.headless === 'boolean' && (
            <Tooltip title={`Browser: ${health.headless ? 'headless' : 'headed (visible)'}`}>
              <Chip
                size="small"
                variant="outlined"
                label={health.headless ? '🕶 headless' : '👁 headed'}
              />
            </Tooltip>
          )}
          {health?.env && (
            <Tooltip title={`Backend in ${health.env} mode`}>
              <Chip
                size="small"
                label={health.env === 'production' ? 'PROD' : 'DEV'}
                color={health.env === 'production' ? 'success' : 'warning'}
              />
            </Tooltip>
          )}
          {health && (
