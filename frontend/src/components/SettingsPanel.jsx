...
export default function SettingsPanel({ open, onClose, health }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const [s, st, u, fp, ai] = await Promise.all([
        getSettings(),
        getHistoryStats(),
        getUsage(),
        listFigmaProjects(),
  ...
  const setBrowser = (key, val) =>
    setSettings((s) => ({ ...s, browser: { ...s.browser, [key]: val } }))
  ...
  const maintenance = async (action, extra, confirmMsg) => {
    ...
    try {
      ...
      setNotice(
        action === 'clear'
          ? `Cleared ${res.removed} report(s)`
          : action === 'purge'
            ? `Purged ${res.removed} old report(s)`
            : action === 'rebuild'
              ? `Index rebuilt (${res.count} report(s))`
              : 'Done',
      )
    } catch (err) {
      setError(err.message)
  ...
  const removeProject = async (id, name) => {
    ...
    setNotice(null)
    try {
      setFigma(await deleteFigmaProject(id))
      setNotice('Figma project removed')
    } catch (err) {
      setError(err.message)
    } finally {
      setSaving(false)
  ...
  const chooseActive = async (id) => {
    ...
    setNotice(null)
    try {
      setFigma(await setActiveFigmaProject(id))
    } catch (err) {
      setError(err.message)
    } finally {
      setSaving(false)
  ...
  const chooseAiModel = async (id) => {
    ...
    setNotice(null)
    try {
      setAiModels(await setActiveAiModel(id))
      setNotice('Audit model updated')
    } catch (err) {
      setError(err.message)
    } finally {
      setSaving(false)
          ...
          <div className="settings-body">
            ...
            <section className="settings-section">
              ...
              <div className="settings-row">
                <span>Backend</span>
                <span className={health?.ok ? 'tag-on' : 'tag-off'}>
              ...
              {KEY_ROWS.map((k) => (
                <div className="settings-row" key={k.id}>
                  <span>
                    {k.label}
                    {k.required && <em className="req"> *</em>}
                  </span>
              ...
              <p className="settings-hint">
                Keys are set in <code>backend/.env</code> and require a server restart.
              </p>
            </section>
            ...
            <section className="settings-section">
              ...
              <p className="settings-hint">
                Add a Figma access token per project. The <strong>active</strong> one is used by
                default; you can also pick a project when configuring a Figma audit. Tokens are
                stored on the backend and shown masked.
              </p>

              {figma.projects.length === 0 && (
                <div className="settings-row">
                  <span style={{ color: 'var(--text-muted)' }}>No project tokens yet.</span>
                </div>
              )}
                ...
                <div className="settings-row" key={p.id} style={{ gap: 8 }}>
                  ...
                  >
                    ...
                    >
                      {p.name}{' '}
                      {p.active && (
                        <span className="tag-on" style={{ marginLeft: 4 }}>
                          active
                        </span>
                      )}
                    </span>
              ...
              <div className="settings-field" style={{ marginTop: 10 }}>
                ...
                <input
                  ...
                  type="text"
                  placeholder="Project name (e.g. Acme Website)"
                  value={newProj.name}
                  onChange={(e) => setNewProj((s) => ({ ...s, name: e.target.value }))}
                />
                <input
                  className="history-search"
                  type="password"
                  ...
                  style={{ marginTop: 8 }}
                  value={newProj.token}
                  onChange={(e) => setNewProj((s) => ({ ...s, token: e.target.value }))}
                />
              </div>
              <button
                className="action-btn primary"
                disabled={saving || !newProj.name.trim() || !newProj.token.trim()}
                onClick={addProject}
              >
            ...
            <section className="settings-section">
              ...
              <label className="settings-field">
                <span>
                  Model {aiModels.activeId ? '(fallback — a profile is active above)' : ''}
                </span>
                <select
                  className="history-filter"
            ...
            <section className="settings-section">
              ...
              <label className="settings-field">
                ...
                <select
                  ...
                  onChange={(e) =>
                    persist({ audit: { ...settings.audit, defaultModule: e.target.value } })
                  }
                >
              ...
              <label className="settings-row" style={{ cursor: 'pointer' }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    persist({ audit: { ...settings.audit, checksAllOn: e.target.checked } })
                  }
                />
              </label>
              <p className="settings-hint">
                Applies the next time you open the app or pick a module.
              </p>
            </section>
            ...
            <section className="settings-section">
              ...
              <p className="settings-hint">
                Disabled tools are not offered to the agent on any audit.
              </p>
              {tools.map((t) => (
                <label
            ...
            <section className="settings-section">
              ...
              <div className="settings-row">
                ...
                <span>{formatNum(usage?.audits)}</span>
              </div>
              <div className="settings-row">
                <span>Input tokens</span>
                <span>{formatNum(usage?.inputTokens)}</span>
              </div>
              <div className="settings-row">
                <span>Output tokens</span>
                <span>{formatNum(usage?.outputTokens)}</span>
              </div>
              <div className="settings-row">
                <span>
                ...
                <span>
                  <strong>{formatNum(usage?.totalTokens)}</strong>
                </span>
              </div>
              {usage?.since && (
              ...
              <div className="settings-actions">
                <button className="history-btn" disabled={saving} onClick={doResetUsage}>
                  Reset counter
                </button>
              </div>
            </section>
            ...
            <section className="settings-section">
              ...
              <div className="settings-row">
                ...
                <span>
                  {stats?.count ?? '—'} · {formatBytes(stats?.totalBytes)}
                </span>
              </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      maintenance(
                        'purge',
                        { days: purgeDays },
                        `Delete all audits older than ${purgeDays} days?`,
                      )
                    }
                  >
                    Purge
              ...
              <div className="settings-actions">
                ...
                <button
                  ...
                  disabled={saving}
                  onClick={() =>
                    maintenance('clear', {}, 'Delete ALL stored audits? This cannot be undone.')
                  }
                >
                  Clear all history
                </button>
