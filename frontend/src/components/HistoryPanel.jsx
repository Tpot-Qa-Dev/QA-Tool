...
export default function HistoryPanel({ open, onClose, onOpenReport, onOpenMerged }) {
  ...
  const doMerge = async () => {
    ...
    setError(null)
    try {
      const full = (await Promise.all([...selected].map(id => getHistoryReport(id)))).filter(Boolean)
  ...
  return (
    <>
      ...
      <aside className={`history-panel ${open ? 'open' : ''}`} role="dialog" aria-label="Audit history">
        <div className="history-head">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              className="ghost-btn"
              onClick={() => { setMergeMode(m => !m); setSelected(new Set()) }}
              style={{ padding: '6px 12px', fontSize: 12, ...(mergeMode ? { borderColor: 'var(--accent)', color: 'var(--accent)' } : {}) }}
        ...
        {mergeMode && (
          <div className="merge-bar" style={{
            ...
            background: 'var(--surface-2)', border: '1px solid var(--accent)', borderRadius: 8,
          }}>
          ...
          {rows.map(r => {
            ...
            return (
              <div
                ...
                style={{ borderLeft: `3px solid ${accent}`, ...(mergeMode && selected.has(r.id) ? { background: 'var(--surface-2)' } : {}) }}
              >
                {mergeMode && (
                  <input
                    type="checkbox"
