...
function MeasuredRow({ label, children }) {
  return (
    ...
    >
      ...
      <span style={{ flex: 1, color: 'var(--text-2)', fontFamily: 'JetBrains Mono, monospace' }}>
        {children}
      </span>
    </div>
  )
  ...
  return (
    <div
      style={{
        ...
        marginBottom: 16,
        borderRadius: 8,
        border: `1px solid color-mix(in srgb, ${e.hue} 40%, transparent)`,
      ...
      <div style={{ fontSize: 12, lineHeight: 1.5 }}>
        ...
        {!environment.isProduction && (
          <div style={{ color: 'var(--text-2)', marginTop: 2 }}>
            Reviewed as <strong>pre-launch</strong> — not marked down for noindex, missing
            analytics, or placeholder content.
          </div>
        )}
    ...
    return (
      <div className="fade-in">
        ...
        <div className="row-end" style={{ justifyContent: 'flex-start' }}>
          <button className="ghost-btn" onClick={onRerun}>
            ↩ Back to Configure
          </button>
          <button className="ghost-btn" onClick={onHome || onReset}>
            🏠 Main menu
          </button>
        </div>
      </div>
      ...
      <div className="card">
        ...
        {report.usage && (
          ...
          >
            ...
            {report.usage.inputTokens?.toLocaleString()} in ·{' '}
            {report.usage.outputTokens?.toLocaleString()} out) · {report.usage.calls} Claude call
            {report.usage.calls === 1 ? '' : 's'}
          </div>
        )}

        <div className="results-summary">
          <div className="summary-card">
            <div className="summary-num" style={{ color: COLORS.pass }}>
              {pass}
            </div>
            <div className="summary-label">✅ Passed</div>
          </div>
          <div className="summary-card">
            <div className="summary-num" style={{ color: COLORS.warn }}>
              {warn}
            </div>
            <div className="summary-label">⚠️ Warnings</div>
          </div>
          <div className="summary-card">
            <div className="summary-num" style={{ color: COLORS.fail }}>
              {fail}
            </div>
        ...
        <div className="card">
          ...
          <div className="recent-urls" style={{ marginTop: 0 }}>
            {report.checks.map((c, i) => (
              <span key={i} className="recent-chip" style={{ cursor: 'default' }}>
                {c}
              </span>
            ))}
          </div>
                ...
                return (
                  <div className="sec-card" key={i} style={{ background: 'var(--surface-2)' }}>
                    ...
                    {s.screenshot && (
                      <img
                        className="sec-shot"
                        src={`data:${s.mimeType || 'image/png'};base64,${s.screenshot}`}
                        alt={s.name}
                      />
                    )}
                    {s.measured && (
                      <div style={{ marginTop: 10 }}>
                        ...
                        <MeasuredRow label="Elements">
                          {c.links ?? 0} links · {c.buttons ?? 0} buttons · {c.images ?? 0} images ·{' '}
                          {c.headings ?? 0} headings{c.forms ? ` · ${c.forms} form` : ''}
                        </MeasuredRow>
                      </div>
                    )}
                  </div>
        ...
        return (
          <div className="card">
            <div className="section-label">
              Findings &amp; Fixes ({findings.length})
              {withShot ? ` · ${withShot} with evidence` : ''}
            </div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 12 }}>
              Click a finding to see the problem, the fix, and a highlighted screenshot where
              available.
            </div>
            {findings.map((f, i) => (
              <FindingAccordion key={i} f={f} />
            ))}
          </div>
        )
      })()}
      ...
      <div className="card">
        ...
        <div className="report-actions" style={{ marginBottom: 14 }}>
          <button
            ...
            style={
              saveState === 'saved'
                ? { background: COLORS.pass, borderColor: COLORS.pass }
                : saveState === 'error'
                  ? { background: COLORS.fail, borderColor: COLORS.fail }
                  : undefined
            }
          >
        ...
        <div className="report-actions">
          <button
            className="action-btn primary"
            onClick={() => exportPdfReport(report, reportId, fileName)}
          >
            📄 Download PDF
          </button>
          <button
            className="action-btn"
            onClick={() => exportHtmlReport(report, reportId, fileName)}
          >
            🌐 Download HTML
          </button>
          <button
            className="action-btn"
            onClick={() => exportMarkdownZip(report, mod?.label, reportId)}
          >
            ⬇ Markdown (.zip)
          </button>
          <button
            className="action-btn"
            onClick={() => exportCsvReport(report, reportId, fileName)}
          >
            📊 Export CSV
          </button>
          <button
            className="action-btn"
            onClick={() => exportJsonReport(report, reportId, fileName)}
          >
            {'{ }'} JSON Export
          </button>
          <button className="action-btn" onClick={() => copyReportSummary(report, reportId)}>
            🔗 Copy Summary
          </button>
        </div>
            ...
            <div className="custom-export-body">
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                <button
                  ...
                  onClick={() => setPicked(new Set(sections.map((s) => s.key)))}
                >
                  Select all
                </button>
                <button className="history-btn" onClick={() => setPicked(new Set())}>
                  Clear
                </button>
                <button
                  className="action-btn primary"
