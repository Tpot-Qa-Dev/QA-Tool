...
const ENVIRONMENTS = [
  { id: 'local',   icon: '🖥️', label: 'Local',         desc: 'Runs on your machine',    placeholder: 'http://localhost:3000' },
  { id: 'staging', icon: '🚧', label: 'Staging / Dev', desc: 'On a dev/staging server', placeholder: 'https://staging.your-site.com' },
  { id: 'live',    icon: '🟢', label: 'Live',          desc: 'Production site',         placeholder: 'https://your-website.com' },
]

export default function ConfigureAudit({
  mod, inputs, setInput, setEnvironment, errors,
  checkState, toggleCheck, setCheckValue, toggleAll, setAllChecks,
  recentUrls = [], recentFigmaUrls = [], figmaProjects = { projects: [], activeId: '' },
  health, canRun, onBack, onChangeModule, onRun,
}) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card" style={{ borderColor: `color-mix(in srgb, ${accent} 33%, transparent)` }}>
        <div className="mod-banner">
          <span className="mod-banner-icon">{mod.icon}</span>
          <div>
            <div className="mod-banner-title" style={{ color: accent }}>{mod.label}</div>
            <div className="mod-banner-desc">{mod.desc}</div>
          </div>
          <button className="ghost-btn" style={{ marginLeft: 'auto', padding: '8px 14px', fontSize: 12 }} onClick={onChangeModule}>
            ← Change
          </button>
        </div>
      </div>
      ...
      {backendDown && (
        <div className="warn-box">✗ Backend offline — start it with <code>cd backend &amp;&amp; npm start</code></div>
      )}
      {noKey && (
        <div className="warn-box">✗ CLAUDE_API_KEY missing — add it to <code>backend/.env</code></div>
      )}
      ...
      <div className="card">
        ...
        <div className="input-group">
          <div className="input-label">🌐 Website URL <span className="tag">required</span></div>
          <input
          ...
          {recentUrls.length > 0 && (
            <div className="recent-urls">
              {recentUrls.slice(0, 5).map(u => (
                <button key={u} type="button" className="recent-chip" title={u}
                  onClick={() => setInput('website_url', u)}>{u}</button>
              ))}
            </div>
          )}
          {env === 'local' && (
            <div className="file-name-hint" style={{ marginTop: 6 }}>
              Tip: you can paste a <code>file:///…/page.html</code> path to audit a local HTML file directly.
            </div>
          )}
        ...
        {needsFigma && (
          <div className="input-group">
            <div className="input-label">⬡ Figma Design URL <span className="tag">required for pixel check</span></div>
            <input
              className="input-field"
              type="url"
            ...
            {recentFigmaUrls.length > 0 && (
              <datalist id="recent-figma-urls">
                {recentFigmaUrls.map(u => <option key={u} value={u} />)}
              </datalist>
            )}
            {recentFigmaUrls.length > 0 && (
              <div className="recent-urls">
                {recentFigmaUrls.slice(0, 5).map(u => (
                  <button key={u} type="button" className="recent-chip" title={u}
                    onClick={() => setInput('figma_url', u)}>{u}</button>
                ))}
              </div>
            )}
            ...
            <div className="input-group" style={{ marginTop: 12 }}>
              ...
              {figmaProjects.projects.length === 0 ? (
                <div className="file-name-hint">
                  No project tokens saved. Add one in <strong>⚙ Settings → Figma Project Tokens</strong> (or set <code>FIGMA_TOKEN</code> in <code>backend/.env</code>).
                </div>
              ) : (
                <select
                  className="input-field"
                ...
                >
                  ...
                  {figmaProjects.projects.map(p => (
                    <option key={p.id} value={p.id}>{p.name} — {p.tokenHint}</option>
                  ))}
                </select>
              )}
            </div>
      ...
      <div className="card">
        <div className="cbx-head-row">
          <div className="section-label" style={{ margin: 0 }}>03 — Select Checks</div>
            ...
            return (
              <div key={g.group}>
                <div className="cbx-group-head">
                  ...
                  {boxes.length > 0 && (
                    <span className="all-toggle" style={{ color: accent }} onClick={() => toggleAll(g, !allOn)}>
                      {allOn ? 'Deselect all' : 'Select all'}
                    </span>
                  )}
                </div>
                {g.items.map(item => isCheckbox(item) ? (
                  <div key={item.id} className="cbx-item" onClick={() => toggleCheck(item.id)}>
                    ...
                    <span className={`cbx-label ${checkState[item.id] ? 'checked' : ''}`}>
                      {item.label}{item.custom && <span className="cbx-custom-tag">custom</span>}
                    </span>
                  </div>
                ) : (
        ...
        <div className="cbx-summary">
          ✓ {selectedCount} {selectedCount === 1 ? 'check' : 'checks'} selected · Claude will focus the audit on these
        </div>
      </div>
      ...
      <div className="row-end">
        ...
        <button
          ...
          disabled={!canRun}
          onClick={onRun}
          style={canRun ? { background: `linear-gradient(135deg, ${accent}, ${accent}BB)` } : undefined}
        >
          ▶ Run Audit
        </button>
      </div>
