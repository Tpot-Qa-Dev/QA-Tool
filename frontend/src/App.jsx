...
import { MODULES, buildCheckState, selectedCheckLabels, selectedCheckTools, mergeCustomChecks } from './config/modules.js'
...
const loadUrlByEnv = () => {
  try { return JSON.parse(localStorage.getItem(URL_BY_ENV_KEY)) || {} } catch { return {} }
}
const saveUrlByEnv = (map) => {
  try { localStorage.setItem(URL_BY_ENV_KEY, JSON.stringify(map)) } catch { /* ignore */ }
}
...
export default function App() {
  ...
  const mod = baseMod ? mergeCustomChecks(baseMod, customChecks[baseMod.id], disabledChecks[baseMod.id]) : null
  ...
  useEffect(() => {
    const raf = requestAnimationFrame(() => { revealAll(); attachTilt() })
  ...
  useEffect(() => {
    getSettings().then(s => {
      ...
      if (ui) {
        ...
        if (!localStorage.getItem('qa-tool-theme') && ui.defaultTheme) theme.setMode(ui.defaultTheme)
      }
    }).catch(() => {})
  }, [])
  ...
  const setEnvironment = (env) => {
    ...
    if (cur) { setUrlByEnv(nextMap); saveUrlByEnv(nextMap) }
  ...
  const toggleAll   = (group, val) =>
    setCheckState(s => {
      const next = { ...s }
      group.items.forEach(i => { if (isCheckbox(i)) next[i.id] = val })
      return next
    })
  ...
  const setAllChecks = (val) =>
    setCheckState(s => {
      const next = { ...s }
      mod?.checkboxGroups.forEach(g => g.items.forEach(i => { if (isCheckbox(i)) next[i.id] = val }))
      return next
    })
  ...
  const websiteError = inputs.website_url.trim()
    ...
    : null
  const figmaFieldError = needsFigma && inputs.figma_url.trim()
    ? validateFigmaUrl(inputs.figma_url)
    : null

  // Figma vs Web also requires the two links to be different.
  const sameUrls =
    needsFigma && !websiteError && !figmaFieldError &&
    inputs.website_url.trim() !== '' && inputs.figma_url.trim() !== '' &&
    normalizeUrl(inputs.website_url) === normalizeUrl(inputs.figma_url)

  const errors = {
    website: websiteError,
    figma:   figmaFieldError || (sameUrls
  ...
  const handleRun = async () => {
    ...
    setUrlByEnv(nextMap); saveUrlByEnv(nextMap)
  ...
  const activeNav = historyOpen ? 'history' : settingsOpen ? 'settings' : adminOpen ? 'admin' : 'dashboard'
  const stepTitle = adminOpen ? 'Admin Dashboard'
    ...
    <div className="app-shell">
      ...
      {step === 4 && (
        <AuditReport
          ...
          reportId={reportId}
          onRerun={() => { setPastReport(null); setStep(2) }}
          onReset={handleReset}
          onHome={handleReset}
        />
      )}
