...
export function useAudit() {
  ...
  const handleEvent = useCallback((event, data) => {
    switch (event) {
      ...
      case 'tool_result':
        ...
        setToolCalls(t => t.map(tc => tc.tool === data.tool ? { ...tc, status: 'done', result: data.result } : tc))
        break
      case 'tool_error':
        addLog(`✗ ${data.tool}: ${data.error}`)
        setToolCalls(t => t.map(tc => tc.tool === data.tool ? { ...tc, status: 'error' } : tc))
        break
      case 'complete':
        setProgress(100)
  ...
  const start = useCallback(async ({ url, figmaUrl, module, checks, requiredTools, reportId, environmentHint }) => {
    setProgress(0)
    setLogs([])
    setToolCalls([])
    setReport(null)
    ...
    try {
      await runAudit({ url, figmaUrl, module, checks, requiredTools, reportId, environmentHint }, handleEvent)
    } catch (err) {
